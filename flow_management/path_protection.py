...
def path_protection(topology, datapaths, hosts,global_flow_table, src_ip, dst_ip, sw_src, sw_dst, msg, add_rules=True):
    ...
    for index, node in enumerate(protected_path):
        ...
        failover_path_creator(topology, datapaths, hosts,global_flow_table,endpoints=endpoints, fine_grain=True, change_arp=False, msg=msg,
...
def failover_path_creator(topology, datapaths, hosts,global_flow_table, endpoints=None, fine_grain=False, modify_rule=False, change_arp=True, msg=None,
    ...
    for endpoint_index, endpoint in enumerate(endpoints):
        ...
        if msg == None:
            ...
            instructions = get_failover_instructions(topology, datapaths, hosts,global_flow_table,src, dst, primary_path, backup_path,in_port)
        else:
            print "Getting failover instructions with msg"
            instructions = get_failover_instructions(topology, datapaths, hosts,global_flow_table,src,dst,primary_path,backup_path,in_port = msg.match['in_port'])

        print "Instructions:",instructions
        install_instructions(topology, datapaths, hosts,global_flow_table,instructions, endpoint, msg,add_rules)
...
def install_instructions(topology, datapaths, hosts,global_flow_table, instructions,endpoint, msg,add_rules=True):
    ...
    for index, sw_src in enumerate(instructions):
        ...
        flow_creator.create_group_mod_failover_flow(datapaths[sw_src], group_id, primary_output_port,
        ...
        if has_backup_path:
            flow_creator.create_l3_failover_flow(datapaths[sw_src], primary_output_port,
                                                 backup_output_port,
                                                 ip_pkt,msg = None, group_id=group_id)
